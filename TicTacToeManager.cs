...
// Gewinnkombinationen der Diagonalen, rechts oben nach links unten
    int[] Diagonale1;
    int[] Diagonale2;
    int[] Diagonale2Ext; // Diagonale 2 extended, also selbe Reihen, nur erst die oberen 3 Buttons, dann die unteren 3 Buttons
    int[] Diagonale4;

// Gewinnkombinationen der Diagonalen, rechts unten nach links oben
    int[] Diagonale5;
    int[] Diagonale6;
    int[] Diagonale6Ext; // Diagonale 6 extended, also selbe Reihen, nur erst die unteren 3 Buttons, dann die oberen 3 Buttons
    int[] Diagonale8;
	...
	void CheckForWinner()
    {
        foreach (int[] combo in winningCombinations)									
        {
			...
			int valA = fieldbuttons[a].FieldValue;
			int valB = fieldbuttons[b].FieldValue;
			int valC = fieldbuttons[c].FieldValue;

																									
			if (valA != -1 && valA == valB && valB == valC)											// Feld muss belegt sein + alle 3 gleich
			{
				int winner = valA + 1;													
				...
				foreach (FieldButton fb in fieldbuttons)                                            // Farbe der Texte in den Buttons ändern
				{
					...
					text.color = new Color32(124, 171, 86, 255);                                    // bei Verwendun von RGB, dann Color32, sonst geht nicht :)
			...
			if (IsBoardFull())
			{
				...
				foreach (FieldButton fb in fieldbuttons)
				{
					...
					text.color = new Color32(124, 171, 86, 255);
																									...
																									//Code, um nach dem drücken des reset buttons den winner text wieder auszublenden:
